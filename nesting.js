...
//   var newEmployeeArr = employees.filter((elem, index, arr) => {
//     // console.log(elem);
...
function removeDuplicates() {
  ...
  for (let i = 0; i < workplaceAccidents.length; i++) {
    if (workplaceAccidents[i + 1] - workplaceAccidents[i] === 0) {
...
function recordCleaner() {
  let newArr = Object.assign({}, myCar);
  for (let i = 0; i < newArr.accidents.length; i++) {
    for (let key in newArr.accidents[i]) {
      if (
        key === 'atFaultForAccident' &&
        newArr.accidents[i].atFaultForAccident === true
      ) {
        newArr.accidents[i].atFaultForAccident = false;
      }
    }
  }
  return newArr;
